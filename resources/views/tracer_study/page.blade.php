...
@section('hero')
  ...
  <section style="background-color: rgb(63, 63, 180);">
    <div class="container" style="padding-top: 200px;">
        <h1 style="color: white;">Hallo, Selamat Mengisi</h1>
    ...
    <div class="container" data-aos="fade-up" style="padding: 20px;">
      ...
      <div class="row justify-content-center " style="padding: 20px;">
        <form action="{{ route('soal1-process') }}" method="POST">
          ...
          <input type="hidden" name="nomer" value="{{$user->nomer}}">
          <input type="hidden" name="type" value="{{$user->type}}">
          <input type="hidden" name="program_studi" value="{{$user->program_studi}}">
          <input type="hidden" name="tahun_lulus" value="{{$user->tahun_lulus}}">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{ route('soal1-process') }}" method="POST">
            @csrf
          <input type="hidden" name="nisn" value="{{$user->nisn}}">
          <input type="hidden" name="nisn" value="{{$user->name}}">
          <input type="hidden" name="nisn" value="{{$user->email}}">
          <input type="hidden" name="nisn" value="{{$user->nomer}}">
          <input type="hidden" name="nisn" value="{{$user->type}}">
          <input type="hidden" name="nisn" value="{{$user->program_studi}}">
          <input type="hidden" name="nisn" value="{{$user->tahun_lulus}}">
            <div class="mb-3">
